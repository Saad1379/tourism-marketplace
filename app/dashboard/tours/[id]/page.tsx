...
import { ArrowLeft, Upload, Plus, Trash2, MapPin, Save, Eye, CheckCircle2, AlertCircle, Clock, Users, Star, GripVertical, ExternalLink, QrCode, Copy } from "lucide-react"
...
export default function EditTourPage() {
  ...
  useEffect(() => {
    const fetchTour = async () => {
      ...
      try {
        ...
        setImages(Array.isArray(tourImages) && tourImages.length > 0 ? tourImages.map((url: string) => ({ url })) : [])
        setHighlights(Array.isArray(tour.highlights) && tour.highlights.length > 0 ? tour.highlights : [""])
        setSelectedLanguages(Array.isArray(tour.languages) && tour.languages.length > 0 ? tour.languages : ["English"])
  ...
  useEffect(() => {
    ...
    const fetchTourQr = async () => {
      ...
      try {
        ...
        if (isMounted) {
          setTourQrUrl(null)
          setTourQrError(error instanceof Error ? error.message : "Unable to load tour QR.")
        }
      } finally {
  ...
  const removeSchedule = (id: string) => {
    // If it's an existing schedule (doesn't start with schedule_ or temp_), track it for deletion
    if (!id.startsWith('schedule_') && !id.startsWith('temp_')) {
  ...
  const publishBlockers = useMemo(() => {
    ...
    if (!formData.city.trim()) issues.push("City is required")
    if (!formData.description.trim()) issues.push("Description is required")
    if (images.length < 1) issues.push("At least 1 photo is required")
    if (schedules.length < 1) issues.push("At least 1 schedule is required")
    if (!formData.meetingPoint.trim()) issues.push("Meeting point is required")
    if (!descriptionMinMet) issues.push(DESCRIPTION_MIN_MESSAGE)
    if (!guideBioMinMet) issues.push(GUIDE_BIO_MIN_MESSAGE)
    if (!stopMinMet) issues.push(MIN_STOPS_MESSAGE)
    if (!futureScheduleMinMet) issues.push(MIN_FUTURE_SCHEDULES_MESSAGE)
    if (!minimumAttendeesValid) issues.push("Minimum attendees cannot be greater than maximum group size")
    return issues
  }, [
    descriptionMinMet,
    formData.city,
  ...
  const handleSubmit = async (status: "draft" | "published") => {
    try {
      ...
      if (status === "published") {
        ...
        if (!formData.title.trim() || !formData.city.trim() || !formData.description.trim()) {
          publishIssues.push("Complete title, city, and description.")
        }
        if (images.length < 1) {
          publishIssues.push("Add at least 1 photo.")
        }
        if (schedules.length < 1) {
          publishIssues.push("Add at least 1 schedule.")
        }
        if (!formData.meetingPoint.trim()) {
          publishIssues.push("Add a meeting point.")
        }
        if (!descriptionMinMet) publishIssues.push(DESCRIPTION_MIN_MESSAGE)
        if (!guideBioMinMet) publishIssues.push(GUIDE_BIO_MIN_MESSAGE)
        if (!stopMinMet) publishIssues.push(MIN_STOPS_MESSAGE)
        if (!futureScheduleMinMet) publishIssues.push(MIN_FUTURE_SCHEDULES_MESSAGE)
      ...
      // Step 1: Handle Deletions first to free up space (limit is 10)
      ...
      // Step 3: Final patch with all metadata + properly ordered photos
      const response = await fetch(`/api/tours/${tourId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      // 2. Identify and Update modified schedules
      const modifiedSchedules = schedules.filter(s => {
        // Only existing schedules can be modified
        if (s.id.startsWith('schedule_') || s.id.startsWith('temp_')) return false
      ...
      const newSchedules = schedules.filter(s => s.id.startsWith('schedule_') || s.id.startsWith('temp_'))
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 sm:gap-4 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs sm:text-sm text-muted-foreground hidden sm:block">Update your tour details</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto py-4 sm:py-8 px-4 sm:px-6">
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              Use this permanent QR for this tour. Guests can scan and review when an attendance-backed session is open.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {tourQrLoading ? (
            ...
            {!tourQrLoading && !tourQrError && tourQrUrl ? (
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                ...
                <div className="flex-1 space-y-3">
                  <p className="break-all rounded-md border bg-muted/40 px-3 py-2 text-sm">{tourQrUrl}</p>
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="grid sm:grid-cols-3 gap-4">
                    ...
                    <Select value={formData.maxGroupSize} onValueChange={(value) => handleInputChange("maxGroupSize", value)}>
                      <SelectTrigger>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="rounded-lg border bg-muted/30 p-4 text-xs text-muted-foreground">
                    ...
                    {compressionMedianBytes && (
                      <p className="mt-1">Current median compressed size: {formatBytes(compressionMedianBytes)}.</p>
                    )}
                  </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  ...
                  <Tabs defaultValue={selectedLanguages[0] || "English"}>
                    <TabsList className="grid w-full gap-1" style={{ gridTemplateColumns: `repeat(${Math.min((selectedLanguages.length > 0 ? selectedLanguages : ["English"]).length, 6)}, minmax(0, 1fr))` }}>
                      {(selectedLanguages.length > 0 ? selectedLanguages : ["English"]).map((lang) => (
                        <TabsTrigger key={lang} value={lang} className="text-xs sm:text-sm truncate">
                          {lang}
                        </TabsTrigger>
                      ))}
                    </TabsList>
