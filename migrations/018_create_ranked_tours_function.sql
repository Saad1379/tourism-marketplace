-- Migration: Create Ranked Tours RPC
-- Description: Core ranking algorithm incorporating all multipliers from the framework
...
BEGIN
  ...
  scored_tours AS (
    SELECT 
      ...
      CASE WHEN tb.is_active = true AND tb.expires_at > NOW() AND (gs.bayesian_rating >= 4.2 OR gs.is_newcomer) THEN
        LEAST(1.25, 1.0 + COALESCE(tb.credits_spent, 50) / 100.0)
      ELSE 1.00 END as boost_mult,
    ...
    LEFT JOIN (
      SELECT b.tour_id, MAX(b.credits_spent) as credits_spent, bool_or(b.is_active) as is_active, MAX(b.expires_at) as expires_at
